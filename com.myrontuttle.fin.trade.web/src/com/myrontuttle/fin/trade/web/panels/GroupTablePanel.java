...
public class GroupTablePanel extends Panel {
	...
	public GroupTablePanel(String id) {
		...
		columns.add(new AbstractColumn<Group, String>(new Model<String>("Alert Period")) {
			@Override
			public void populateItem(Item<ICellPopulator<Group>> cellItem,
					String componentId, IModel<Group> groupModel) {
				if (groupModel.getObject().getStringSettings().containsKey("Alert.Period")) {
					cellItem.add(
						new Label(componentId, groupModel.getObject().getString("Alert.Period")));
				} else {
					cellItem.add(new Label(componentId, ""));
				}
			}
